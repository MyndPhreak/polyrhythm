<template>
  <div class="relative w-full h-[600px] group flex justify-center">
    <canvas
      ref="canvasRef"
...
const currentScaleInfo = computed(() => {
  console.log(`Visualizer: Computing currentScaleInfo with rootNote=${settings.value.rootNote}, scaleType=${settings.value.scaleType}`);
...
const initializeVisualizationNodes = () => {
  ...
  for (let i = 0; i < nodeCount; i++) {
    const nodeState: CanvasNode = {
      position: 0.0, // Start at bottom (normalized 0-1)
